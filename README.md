...
| PUT | `/api/config` | 更新 + 持久化 + 热更新 |
| GET | `/api/status` | 代理 / Zen 上游健康状态 |
| GET | `/api/requests` | 最近的 `/v1/messages` 调用（含路由协议） |
| GET | `/api/models` | Zen 模型目录（49 个） |
| GET | `/api/stats` | 小时级 + 分模型 + 汇总 |
| GET | `/api/test` | ping Zen（GET /v1/models） |
